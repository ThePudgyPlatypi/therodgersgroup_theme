	...
	<div class="main-grid">
		...
		<?php if(is_page("Accreditation Services")) { ?>
			<header class="featured-hero half-hero" role="banner">
				...
				<div class="orbit half-header" role="region" aria-label="accreditation services header slider" data-orbit data-options="animInFromLeft:fade-in; animInFromRight:fade-in; animOutToLeft:fade-out; animOutToRight:fade-out;">
		...
		<?php } elseif(is_page("corporate-values")) { ?>
			<header class="featured-hero half-hero-video" role="banner" style="background-image: url(<?php the_post_thumbnail_url(); ?>)">
				<video playsinline autoplay muted loop class="hero-video" autoplay loop poster="<?php the_post_thumbnail_url(); ?>">
					<source src="<?php bloginfo('template_url'); ?>\dist\assets\video\corporateValues-small.mp4" type="video/mp4">
